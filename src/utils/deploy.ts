...
export async function verifyOnEtherscan(contract: any, constructorArgumentsEncoded: string, etherscanAPIKey: string): Promise<any> {
  ...
  // Etherscan does not like multiple SPDX identifies in the flattened source
  ...
  const licensesRemovedSource = contractSource.replace(/^\/\/ SPDX-License.*/mg, '');
  ...
  const verifierOptions = {
    ...
    contractSource: licensesRemovedSource
  };
  ...
  await fs.writeFile(flattenFile, licensesRemovedSource);
